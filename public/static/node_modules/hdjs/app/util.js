    ...
    util = {
        ...
        zclip: function (elem, content, callback) {
            ...
            require(['clipboard'], function (Clipboard) {
                ...
                elem.clipboard.on('success', function (e) {
                    elem = e.trigger;
                    if ($.isFunction(callback)) {
                        return callback(elem, content);
                    } else {
                        var obj = $(elem).next('.hdJsCopyElem');
                        if (obj.length == 0) {
        ...
        emotion: function (options) {
            require(['caret', 'bootstrap', 'css!../css/emotion.css'], function ($) {
                $(function () {
                    ...
                    if ($(btn).attr('emotions_action')) {
                        return;
                    }
                    $(btn).attr('emotions_action', true);
                    $(btn).popover({
                        html: true,
        ...
        validCode: function (option) {
            option = $.extend({
                //后台链接
                url: '?s=site/tool/sendValidCode&m=ucenter&siteid=' + window.system.siteid,
                //验证码等待发送时间
                timeout: 60,
            }, option);
            require(['jquery', 'util'], function ($, util) {
                ...
                var obj = {
                    ...
                    update: function () {
                        var This = this;
                        $.get('?s=site/tool/getMessageCodeDiffTime&m=ucenter&siteid=' + window.system.siteid, function (times) {
