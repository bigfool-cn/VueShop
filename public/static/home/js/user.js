...
function checkEmail(email)
{
  ...
  if (email == '')
  {
	  document.getElementById('email_notice').innerHTML = '<font color="#939393"> 邮箱/手机是您的登陆凭据</font>';
	  return;
...
function check_email_callback(result)
{
  if ( result == 'ok' )
  {
    ...
    if(typeof(document.forms['formUser']) != 'undefined'){
    	document.forms['formUser'].elements['Submit'].disabled = '';
    }
  }
  else
...
function getExpressInfo(orderId){
	...
	jQuery.post('user.php?act=get_express_info&is_ajax=1&new=1', 'order_id=' + orderId, getExpressInfoResponse,'JSON' );
